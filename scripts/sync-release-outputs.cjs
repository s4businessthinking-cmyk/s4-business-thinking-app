...
function writeInstallReadme(targetDir, version) {
  ...
  const text = [
    ...
    "  Updates are automatic (like WhatsApp). No uninstall. App restarts itself when ready.",
    "",
    "Android Mobile:",
    `  Install once: ${RELEASE_CONFIG.fileNames.apk(version)}`,
    "  After first install with OTA, future updates apply automatically in the background.",
    "",
